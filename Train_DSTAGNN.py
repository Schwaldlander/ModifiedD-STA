...
from lib.utilsVal import load_graphdata_channel1, get_adjacency_matrix2, compute_val_loss_mstgcn, predict_and_save_results_mstgcn#utilsVal.py
...
print('WandB project Name',run_name)
confo = wandb.config##confo
def seed_torch(seed):
    random.seed(seed)
...
parser.add_argument("--config-path", default='./configurations/PEMS04_dstagnn.conf', type=str,
                    help="configuration file path")#add statement of path
args = parser.parse_args()
config = configparser.ConfigParser()
print('Read configuration file: %s' % (args.config_path))#############
...
#the files are generated prior to this, see STAG_gen.py  
...
confo.in_channels=in_channels
confo.learning_rate=learning_rate
confo.nb_block= nb_block
confo.nb_chev_filter= nb_chev_filter
confo.nb_time_filter= nb_time_filter
confo.time_strides=time_strides
confo.batch_size=batch_size
confo.graph_signal_matrix_filename=graph_signal_matrix_filename
confo.start_epoch=start_epoch
confo.epochs=epochs




##@hydra.main(config_path="configurations", config_name="confi.yaml")###
def train_main():
...
#        Wandb.launch(cfg, cfg.log, True)#what is it?
#        #for weight and bias analysis
#        get_logger(
#            "jam_.log",
#            clear=True,
...
#    jampt.set_gpu_mode(cfg.cuda, cfg.trainer.gpu)
    ...
    print(net)#from here we can see the net structure clearly
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for batch_index, batch_data in enumerate(train_loader):
            ...
            wandb.log({"Val Loss": val_loss,"Training Loss": training_loss})
...
if __name__ == "__main__":
    ...
    train_main()
    #predict_main(18, test_loader, test_target_tensor, _mean, _std, 'test')#try to predict
